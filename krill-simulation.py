...
def calc_krill_biomass(survey_area_width, survey_area_length, edsu_width, detected_width):
    ...
    edsu[np.random.choice(range(0, int(n_cells)), size=n_cells*0.75, replace=False)] = np.hstack([np.random.exponential(5, n_cells*0.55), np.random.exponential(100, n_cells*0.2)])
...
print np.max(krill_biomass - calculated_biomass)/np.mean(krill_biomass) * 100