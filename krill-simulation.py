...
def calc_krill_biomass(survey_area_width, survey_area_length, edsu_width, detected_width):
    ...
    edsu = np.random.exponential(5, n_cells)
    edsu[np.random.choice(range(0, int(n_cells)), size=n_cells*0.2, replace=False)] = np.random.exponential(100, n_cells*0.2)
    edsu[np.random.choice(range(0, int(n_cells)), size=n_cells*0.25, replace=False)] = 0
